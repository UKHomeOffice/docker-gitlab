module Backup
  class Manager
    ...
    def encrypt(tar_file)
      uid = Gitlab.config.backup.encryption.uid
      $progress.print "Encrypting archive: #{tar_file} ... "
      if RubyGpg.encrypt(tar_file, uid)
        $progress.puts "done".green
      else
    ...
    def decrypt(tar_file)
      passphrase = Gitlab.config.backup.encryption.passphrase
      $progress.print "decrypting archive: #{tar_file} ... "
      if RubyGpg.decrypt(tar_file+'.gpg', passphrase)
        $progress.puts "done".green
      else
          puts "Decrypting archive #{tar_file} failed".red
          abort 'Decypting backup failed'
      end
    end
